...
# hess and grad reader adapted from https://github.com/glibaniosr/ORCA-read-for-glowfreqs/blob/master/orcaread-gf.py
from os import read
...
def read_orca_hess_grad(hess_file, grad_file):
        ...
        for line in inp:
            if not "$atoms" in line: # Flag for the Hessian group
                ...
                for idx,data in enumerate(inp):
                    ...
                    masses.append(float(data[1]))
                    xyz_data[idx] = [float(data[2]), float(data[3]), float(data[4]) ]


    grad_data = make_array(n_atoms,3)
        ...
        for line in inp:
            if not "gradient in Eh/bohr" in line: # Flag for gradient group, and possibility to write on file.
                ...
                for idx,data in enumerate(inp):
                    ...
                    if (idx+1)%3 == 0:
                        nline = (idx+1)//3 - 1
                        grad_data[nline] = [float(xyz_temp[0]), float(xyz_temp[1]), float(xyz_temp[2]) ]
                        xyz_temp = []
    ...
    numbers = np.zeros(len(elements), int)
    for index, element in enumerate(elements):
        number = periodic[element].number
        numbers[index] = number

    # orca is molecular data
    rvecs = None
    pbc = [0,0,0]

    return(numbers, np.array(xyz_data), float(energy), np.array(grad_data), hess_data, np.array(masses), rvecs, pbc)
